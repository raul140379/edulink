...
const TOKEN_KEY = 'regente_token'
const USER_KEY = 'regente_user'
...
export async function login(email: string, password: string): Promise<{ ok: true } | { ok: false; message: string }> {
  try {
    ...
    if (!ALLOWED_ROLES.includes(data.user.role)) {
      return { ok: false, message: 'Esta app es solo para Regente. Usá el panel web con tu cuenta.' }
    }
