...
**URGENTE-ALTA → ACTUALIZADO 5-sep-2026: diseño completo por bloques CONSTRUIDO Y PROBADO EN LOCAL, no bloquea nada (no desplegado todavía, sin apuro real de por medio):**
- `AttendanceBlock` real + `subjectId` como corte adicional + Licencia de estudiante (Opción A, solo alta) + backend de la matriz semanal — todo construido y probado contra el backend local con casos reales. Falta: PDF por bloque, y frontend/PDF de la matriz semanal (con maqueta, como se acordó) — y luego desplegar (LOCAL ya probado → falta CLON → PRODUCCIÓN, avisar a Raul antes de arrancar ese camino). Detalle completo en "Prioridad 1.5" más abajo.
...
### 🟠 Prioridad 1.5 — Asistencia: rediseño por bloques de períodos (mitigación + fix de fondo cerrados 4-sep-2026; diseño completo por bloques CONSTRUIDO Y PROBADO EN LOCAL 5-sep-2026, sin desplegar todavía)
...
**Diseño completo por bloques — CONSTRUIDO Y PROBADO EN LOCAL (5-sep-2026), NO DESPLEGADO. Avisar a Raul para revisión antes de cualquier deploy.**

Regla de agrupación confirmada: si un maestro tiene varios períodos SEGUIDOS (números de período consecutivos — un recreo entre período 2 y 3 no rompe el bloque, se interpreta por número, no por reloj real) con el mismo curso el mismo día, es UNA sola asistencia para todo el bloque. Períodos no consecutivos del mismo maestro/curso/día = bloques separados. Un cambio de materia (`subjectId`) también corta el bloque, aunque los períodos sean numéricamente consecutivos — investigado con datos reales antes de asumir: hoy el generador de horarios nunca pone 2 materias del mismo maestro+curso el mismo día, así que en la práctica este corte no se dispara, pero no está garantizado por el modelo (un horario editado a mano sí podría hacerlo), así que se agregó por seguridad estructural. Un solo período = bloque de tamaño 1, mismo algoritmo, sin caso especial.

1. ✅ **Algoritmo de bloques** (`backend/src/utils/attendance-blocks.ts`, `groupIntoBlocks`) — corta por período no consecutivo Y por cambio de `subjectId`. Unit-testeado con casos sueltos (período único, consecutivos misma materia, consecutivos materia distinta, input desordenado, vacío).
2. ✅ **Modelo de datos — Opción B** — tabla `AttendanceBlock` real (`courseId, teacherId, subjectId?, date, periodStart, periodEnd, startTime, endTime, academicYearId, schoolId`) + `StudentAttendance.blockId` FK nullable (`@@unique([studentId, blockId])`, reemplaza la clave por `teacherId` del fix de esta noche del 4-sep) + `StudentAttendance.teacherId` denormalizado se mantiene. `subjectId` nullable: el bloque sentinela "sin período" (camino DIRECTOR/SECRETARY sobre un maestro sin ningún período programado ese día) no tiene materia real detrás. El bloque se resuelve y se **congela en el momento de guardar** (`findOrCreateAttendanceBlock`, `upsert` con `update: {}`, no-op real) — nunca se recalcula en vivo si el horario se edita después. Migraciones `20260904115013_attendance_blocks` y `20260904205127_attendance_block_subject_and_licenses`, aplicadas y verificadas sin drift SOLO en local.
2.1. ✅ **Licencia de estudiante (Opción A)** — tabla `StudentLicense` nueva (`studentId, startDate, endDate, reason?, createdById, schoolId`), pantalla de alta para Dirección: `POST /api/student-licenses` (gateado `DIRECTOR`/`SECRETARY`, mismo criterio que el resto de correcciones sin ventana horaria; REGENTE queda afuera a propósito). **Nunca escribe `StudentAttendance`** — se consulta en batch en cada punto de lectura (`getAttendanceByCourse`, `closeAttendance`, matriz semanal) y "tapa" el estado resuelto de ese estudiante ese día con `LICENCIA`, sin tocar ni borrar la fila real de abajo si el maestro ya había registrado algo. `closeAttendance` excluye a los estudiantes de licencia del auto-`AUSENTE` (ya están justificados, no les "falta" nada). Probado de punta a punta contra el backend local: licencia creada para un estudiante real de un curso real, vista tapada correctamente (status `LICENCIA`, `onLicense:true`, `note` con el motivo) sin generar ninguna fila `StudentAttendance` nueva, `closeAttendance` marcó AUSENTE a 36 de 37 (excluyó exacto al de licencia), verificado también que la fila histórica previa del estudiante (de antes de la licencia) queda intacta. Limpiado sin rastro después.
2.2. **Alcance confirmado de Licencias, SOLO ALTA por ahora** — listado/anulación de licencias ya creadas queda pendiente, sin apuro, decisión explícita de Raul (5-sep-2026) para no repetir el mismo hueco que quedó en Gobierno Estudiantil (alta sin gestión posterior, ver Prioridad 4 abajo). Cuando se retome: pantalla de listado por estudiante/rango + acción de anular (borrado real o cierre de `endDate`, a decidir en su momento).
3. ✅ **Matriz semanal (backend, sin frontend/PDF todavía)** — `GET /api/reports/attendance-weekly/:courseId` (`report.service.ts`, `getWeeklyAttendanceMatrix`), 5 consultas totales sin importar cuántos bloques tenga la semana (horario completo de la semana + `AttendanceBlock` reales + roster + asistencia de esos bloques + licencias superpuestas), nunca una por día/bloque. Filas=bloques, columnas=días (confirmado con Raul), con licencias aplicadas por celda. Probado contra un curso real de la semana actual: días con varios maestros/materias distintos se agrupan bien, un día con 2 tramos del mismo maestro (si lo hubiera) los separa, y la licencia de prueba se reflejó como `licencias:1`/`sinRegistrar:36` en LOS 3 bloques del día, no solo en el que ya estaba cerrado — confirma que el "tapado" por licencia aplica por celda, no solo al bloque tocado. **Sin frontend ni PDF todavía — eso se diseña con una maqueta aparte, como se acordó explícitamente, no construir a ciegas.**
4. ⏳ **PDF con firma — pendiente, hoy sigue exportando el modo simple** (el bloque actual del maestro que exporta, sin selector): un PDF por bloque — un maestro con 3 períodos seguidos tendría una sola firma para todo el bloque. Encabezado agregaría el rango ("Períodos 3°-5° · 09:35-11:30"). Ver también el PDF semanal "reporte gerencial sin firma" (distinto de este), confirmado pero no construido — depende del frontend de la matriz (punto 3).
5. ✅ **Ventana de horario del Maestro** — sin cambios, ya estaba bien.
6. ✅ **Gamificación** — sin cambios respecto al fix del 4-sep, sigue resuelto (`collapseToDailyStatus`).
7. ✅ **Notificación a padres** — sin cambios respecto al fix del 4-sep, sigue resuelto.
8. ✅ **`closeAttendance`** — ahora scopeado por `blockId` real (antes por `teacherId` suelto) + excluye licencias (ver 2.1).

**Estado real: el diseño completo por bloques queda funcionalmente cerrado y probado en local (incluida la interacción con Licencias) — el único trabajo de código que falta es la UI/PDF de la matriz semanal (punto 3, con maqueta) y el PDF por bloque (punto 4). Nada de esto (bloques, licencias, matriz) se desplegó todavía — sigue 100% en local, pendiente de aviso explícito a Raul antes de cualquier deploy (mismo protocolo de siempre: LOCAL → CLON → PRODUCCIÓN con confirmación entre cada paso, en cuanto se decida desplegar).**
