...
28. ⏳ **Diagnóstico pendiente del resto de módulos — ¿queda más trabajo de migración a producción?** Con Estudiantes/Padres/Tesorería ya migrados (ver 19.8), falta el mismo ejercicio de diagnóstico para el resto: **Académico** (Nota, Task/TaskSubmission, StudentAcademicAssignment, Course, Subject, TeacherSubjectCourse, Schedule/SchedulePlan), **Asistencia** (StudentAttendance, TeacherAttendance, Meeting/Attendance, Convocatoria/ConvocatoriaAttendance), **Portería** (GateRecord, BiometricTemplate), **Comunicación** (Notification, Comunicado), **Gobernanza** (JuntaMember, GobiernoMember más allá de lo ya migrado en Padres), **Docentes/Staff** (Teacher, Staff, sus asignaciones). Mismo criterio que Tesorería: conteo total en local vs producción por tabla, para saber si hay datos reales sin migrar o si esos módulos simplemente no tienen uso real todavía (plausible para varios — el sistema lleva poco tiempo en producción real). **No ejecutar todavía** — siguiente paso lógico después de cerrar el trabajo en curso, no urgente.
...
## 🔖 CIERRE DE SESIÓN 24-ago-2026 — estado exacto para retomar

**MIGRACIÓN LOCAL→PRODUCCIÓN: ✅ COMPLETA, sin nada pendiente.** Los 4 componentes del plan original quedaron cerrados y verificados 1:1 contra producción real:
...
**No queda ningún dato real de local sin su contraparte en producción.**

**Pendientes de fondo, sin urgencia, ya anotados en el Roadmap** (ninguno bloquea nada, se retoman cuando haga falta): **20** (probar en pantalla todo lo migrado — hasta ahora solo verificado por API/DB), **21** (checklist `docs/reporte-pre-produccion-tesoreria-2025.md`), **24.1** (frontend debe pedir páginas para que la paginación reduzca carga real), **26** (evaluar PgBouncer cuando haga falta más de ~8 instancias), **27** (LICENSE propio + postura sobre copyleft débil, decisión de negocio), **28** (diagnóstico de conteos local vs producción para el resto de módulos — Académico/Asistencia/Portería/Comunicación/Gobernanza/Docentes — siguiente paso lógico después de esta ronda, no ejecutado todavía), **17.1** (auditoría/trazabilidad de acciones destructivas), **17.2** (2 `Charge` PAGADO sin `Payment`, Bs. 200, deuda de datos preexistente), **18** (dominio de negocio de Tesorería Núcleo/Distrito), **19.2.2** (root cause del evento 21-jun que vació 31 `rude`, sin confirmar), **4 excluidos de Fase 4 por tutor placeholder** (`Student.id` 567, 572, 601, 623, ver 19.2.3), **4 casos de Fase 4 sin crear todavía** (Winder no aplica — ver 19.3 — pero Jorge/Karina/Lina de padres ya se crearon el 23-ago; los estudiantes genuinamente nuevos — los 8 de Fase 1 — resultaron ser duplicados, no altas, ver 19.3.2, así que no queda ninguna alta pendiente de Fase 4 real).
