  ...
  return (
    <div>
      <div style={{marginBottom:24}}>
        <h1 style={{fontSize:20,fontWeight:700,color:'#1A3A7C',marginBottom:4}}>Reporte de Accesos</h1>
        <p style={{fontSize:13,color:'#6B8BB0'}}>Historial de entradas y salidas registradas por el portero</p>
      </div>

      {/* Filtros */}
      <div style={{background:'#fff',border:'1px solid #CBE0F0',borderRadius:12,padding:'14px 18px',marginBottom:20,display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
        <div style={{display:'flex',flexDirection:'column',gap:5}}>
          <label style={{fontSize:11,fontWeight:600,color:'#1A3A7C',textTransform:'uppercase',letterSpacing:'.5px'}}>Fecha</label>
          <input type="date" value={date} onChange={e=>setDate(e.target.value)}
            style={{padding:'8px 12px',border:'1.5px solid #CBE0F0',borderRadius:8,fontSize:13,color:'#1A3A7C',outline:'none'}}/>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:5}}>
          <label style={{fontSize:11,fontWeight:600,color:'#1A3A7C',textTransform:'uppercase',letterSpacing:'.5px'}}>Tipo</label>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:5}}>
          <label style={{fontSize:11,fontWeight:600,color:'#1A3A7C',textTransform:'uppercase',letterSpacing:'.5px'}}>Acción</label>
        ...
        <button onClick={loadRecords} style={{
          marginTop:18,padding:'8px 16px',background:'#1A3A7C',color:'#fff',
      ...
      {summary && (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:12,marginBottom:20}}>
          {[
            {label:'Total registros', value:summary.total,          color:'#1A3A7C', bg:'#E0ECF8'},
      ...
      <div style={{background:'#fff',border:'1px solid #CBE0F0',borderRadius:12,overflow:'hidden'}}>
        {loading ? (
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {records.map(r => (
                <tr key={r.id} style={{borderTop:'1px solid #F0F6FC'}}>
                  <td style={{...td,fontWeight:700,color:'#1A3A7C',whiteSpace:'nowrap'}}>
                  ...
                  <td style={td}>
                    <span style={{
                      ...
                      color:r.type==='MAESTRO'?'#1A3A7C':'#0F6E56',
                    }}>
                      {r.type==='MAESTRO'?'👨‍🏫 Maestro':'🧑‍💼 Visitante'}
                    </span>
                  </td>
                  <td style={td}>
                    <div style={{fontWeight:600,fontSize:13,color:'#1A3A7C'}}>
                      {r.teacher ? `${r.teacher.lastName} ${r.teacher.firstName}` : r.visitorName}
                    </div>
                    {r.teacher?.specialty && (
                      <div style={{fontSize:11,color:'#6B8BB0'}}>{r.teacher.specialty}</div>
                    )}
                    {r.visitorCI && (
                      <div style={{fontSize:11,color:'#6B8BB0'}}>CI: {r.visitorCI}</div>
                    )}
                  </td>
                  <td style={td}>
                    <span style={{
                  ...
                  <td style={{...td,fontSize:12,color:'#6B8BB0'}}>
