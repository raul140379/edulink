...
export default function TeacherWorkloadPage() {
  ...
  if (error || !data) return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:80,gap:12,color:'#6B8F7F'}}>
      <p style={{color:'#C0392B'}}>{error || 'No se encontraron datos'}</p>
      <button onClick={() => router.back()} style={{padding:'8px 16px',background:'#0A5A45',color:'#fff',border:'none',borderRadius:8,cursor:'pointer'}}>
        Volver
      </button>
    </div>
  )
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'flex-start',gap:16,marginBottom:24,flexWrap:'wrap'}}>
        <button onClick={() => router.back()} style={{
          ...
          cursor:'pointer',color:'#6B8F7F',fontSize:13,padding:'6px 0',whiteSpace:'nowrap'
        }}>
          <ArrowLeft size={16}/> Volver
        </button>
        <div style={{flex:1}}>
          <h1 style={{fontSize:22,fontWeight:700,color:'#0A5A45',margin:'0 0 4px'}}>
            {data.teacher.lastName} {data.teacher.firstName}
          </h1>
          <p style={{fontSize:13,color:'#6B8F7F',margin:0}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:14,marginBottom:24}}>
        <div style={{background:'#0A5A45',color:'#fff',borderRadius:14,padding:'20px',textAlign:'center'}}>
      ...
      {data.totalesPorHorario && data.totalesPorHorario.length > 0 && (
        <div style={{marginBottom:24}}>
          <h2 style={{fontSize:15,fontWeight:700,color:'#0A5A45',marginBottom:12,textTransform:'uppercase',letterSpacing:'.5px',display:'flex',alignItems:'center',gap:8}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:14}}>
            {data.totalesPorHorario.map(h => {
              ...
              return (
                <div key={h.horarioId} style={{background:'#fff',border:'1px solid #DCEEE6',borderRadius:14,padding:20}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                    <div>
                      <div style={{fontSize:16,fontWeight:700,color:'#0A5A45'}}>
                        {h.isWinter ? '❄️' : '☀️'} {h.nombre}
                      </div>
                      <div style={{fontSize:12,color:'#6B8F7F',marginTop:4}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:16}}>
                    <div style={{background:'#F8FBFF',borderRadius:10,padding:'12px',textAlign:'center'}}>
                      <div style={{fontSize:24,fontWeight:800,color:'#0A5A45'}}>{h.totalPeriodos}</div>
                      <div style={{fontSize:11,color:'#6B8F7F',marginTop:2}}>periodos/sem</div>
                    </div>
                    <div style={{background:'#F8FBFF',borderRadius:10,padding:'12px',textAlign:'center'}}>
                      <div style={{fontSize:24,fontWeight:800,color:'#0A5A45'}}>{h.horasSemana}</div>
                      <div style={{fontSize:11,color:'#6B8F7F',marginTop:2}}>hrs/semana</div>
                    </div>
                    <div style={{background:'#F8FBFF',borderRadius:10,padding:'12px',textAlign:'center'}}>
                      <div style={{fontSize:24,fontWeight:800,color:'#0A5A45'}}>{h.horasMes}</div>
                      <div style={{fontSize:11,color:'#6B8F7F',marginTop:2}}>hrs/mes</div>
                    </div>
                  </div>
                  {/* Barra progreso */}
                  <div style={{background:'#F5FAF7',borderRadius:20,height:8,overflow:'hidden',marginBottom:6}}>
                    <div style={{
                  ...
                  <div style={{display:'flex',justifyContent:'space-between'}}>
                    <span style={{fontSize:11,color:'#6B8F7F'}}>0 hrs</span>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {data.assignments.map((a, i) => (
                <tr key={i} style={{borderTop:'1px solid #F5FAF7'}}>
                  <td style={{...td,color:'#6B8F7F',fontSize:12}}>{i + 1}</td>
                  ...
                  <td style={{...td,textAlign:'center'}}>
                    <span style={{
                      background: a.periodosAsignados > 0 ? '#E0ECF8' : '#F5FAF7',
                      color:      a.periodosAsignados > 0 ? '#0A5A45' : '#6B8F7F',
            ...
            <tfoot>
              <tr style={{borderTop:'2px solid #DCEEE6',background:'#F8FBFF'}}>
                <td colSpan={4} style={{padding:'12px 14px',fontWeight:700,color:'#0A5A45',fontSize:13}}>
                  Total
                </td>
                <td style={{padding:'12px 14px',textAlign:'center'}}>
                  <span style={{background:'#E0ECF8',color:'#0A5A45',padding:'3px 12px',borderRadius:20,fontSize:13,fontWeight:700}}>
      ...
      <style>{`
        .spinner{width:28px;height:28px;border:3px solid rgba(10,90,69,.2);border-top-color:#0A5A45;border-radius:50%;animation:spin .7s linear infinite}
...
const th: React.CSSProperties = {
  ...
  color:'#0A5A45', textTransform:'uppercase', letterSpacing:'.5px', whiteSpace:'nowrap'
}
const td: React.CSSProperties = {
  padding:'11px 14px', fontSize:13, color:'#0A5A45', verticalAlign:'middle'
}