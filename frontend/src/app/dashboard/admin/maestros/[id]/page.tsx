...
import {
  ...
  User, Save, X, GraduationCap, CheckCircle2
} from 'lucide-react'
...
interface Assignment {
  ...
  course: {
    id: number; grade: string; parallel: string
    level: string; shift: string; educationType: string
  }
}
...
export default function TeacherDetailPage() {
  ...
  const fetchSpecialties = async () => {
    try {
      const res  = await fetch(`${API_URL}/api/teachers/${id}/specialties`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await res.json()
  ...
  const handleAssign = async () => {
    if (!selectedCourse || !selectedSubject) {
      notify('Selecciona un curso y una materia', 'err'); return
    }
    setSaving(true)
    try {
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <button className="btn-primary" onClick={openModal}>
          <Plus size={15} /> Agregar asignación
        </button>
      </div>
      ...
      <div className="info-card">
        <div className="info-row">
          ...
          <span className="info-val">{teacher.ci || '—'}</span>
        </div>
        <div className="info-row">
          <span className="info-label">Teléfono:</span>
          <span className="info-val">{teacher.phone || '—'}</span>
        </div>
        <div className="info-row">
          <span className="info-label">Email:</span>
          <span className="info-val">{teacher.user?.email || teacher.email || '—'}</span>
        </div>
        <div className="info-row">
          ...
          <span className="info-val">
            {teacher.hoursLoad
              ? <span className="hrs-badge">{teacher.hoursLoad} hrs</span>
              : '—'}
          </span>
        </div>
        <div className="info-row">
          ...
          <span className="info-val">
            <strong>{grouped.length}</strong> cursos · <strong>{totalMaterias}</strong> materias
          </span>
        </div>
      </div>

      {/* Asignaciones agrupadas por curso */}
      ...
      {grouped.length === 0 ? (
        <div className="empty-state">
          ...
          <button className="btn-primary" onClick={openModal}>
            <Plus size={14} /> Agregar primera asignación
          </button>
        </div>
      ) : (
            ...
            <div className="course-header">
              <div className="course-title">
                ...
                {course.educationType === 'BTH' && (
                  <span className="course-badge bth">BTH</span>
                )}
              </div>
                ...
                <div key={a.id} className="subject-row">
                  <div className="subject-info">
                    <span
                      className="campo-dot"
                      style={{
                        background: CAMPO_COLORS[a.subject.campo || ''] || '#F0F0F0',
                        color:      CAMPO_TEXT[a.subject.campo  || ''] || '#444',
                      }}
                    >
        ...
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="mbody">
              ...
              {/* Selector de materia — solo especialidades del maestro */}
              <div className="fg">
                <label>Materia *</label>
                {specialties.length === 0 ? (
                  <div className="warn-box">
                    ⚠️ Este maestro no tiene materias registradas como especialidad.
                    Agrégalas primero desde la lista de maestros.
                  </div>
                ) : (
                  <select
                  ...
                  >
                    ...
                    {specialties.map(sp => (
                      <option key={sp.subject.id} value={sp.subject.id}>
                        {sp.subject.name}
                        {sp.subject.campo ? ` · ${CAMPO_LABELS[sp.subject.campo] || sp.subject.campo}` : ''}
                      </option>
                    ))}
                  </select>
                )}
                ...
                >
                  ...
                  {courses.map(c => (
                    <option key={c.id} value={c.id}>
                      {GRADES[c.grade]} &quot;{c.parallel}&quot; · {SHIFTS[c.shift]} · {LEVELS[c.level]}
              ...
              {previewCourse && previewSubject && (
                <div className="preview-box">
                  ...
                  <span>
                    <strong>{teacher.firstName}</strong> enseñará{' '}
                    <strong>{previewSubject.subject.name}</strong> en{' '}
                    <strong>
                      {GRADES[previewCourse.grade]} &quot;{previewCourse.parallel}&quot; {SHIFTS[previewCourse.shift]}
                    </strong>
                  </span>
                </div>
              )}
            ...
            <div className="mfoot">
              ...
              <button
                ...
                disabled={saving || !selectedCourse || !selectedSubject}
              >
      ...
      <style>{`
        ...
        .course-title{display:flex;align-items:center;gap:8px;font-weight:700;color:#1A3A7C;font-size:14px}
        ...
        .modal{background:#fff;border-radius:14px;width:100%;max-width:440px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.15);display:flex;flex-direction:column}
        ...
        .fg-select{padding:9px 12px;border:1.5px solid #CBE0F0;border-radius:8px;font-size:13px;color:#1A3A7C;outline:none;width:100%;background:#fff}
        .fg-select:focus{border-color:#4A9FD4;box-shadow:0 0 0 3px rgba(74,159,212,.12)}
        .fg-select:disabled{opacity:.5;cursor:not-allowed;background:#F8FBFF}
