  ...
  return (
    <div>
      ...
      <div style={{marginBottom:20}}>
        <h1 style={{fontSize:20,fontWeight:700,color:'#00838F',marginBottom:4}}>Horario Escolar</h1>
      ...
      {students.length > 1 && (
        <div style={{background:'#fff',border:'1px solid #CBE0F0',borderRadius:10,padding:'12px 16px',marginBottom:16,display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
          <span style={{fontSize:12,fontWeight:700,color:'#00838F'}}>Seleccionar hijo:</span>
          {students.map(s=>(
            <button key={s.id} onClick={()=>setSelStudent(s)} style={{
              padding:'6px 14px',borderRadius:20,border:'none',cursor:'pointer',fontSize:12,fontWeight:600,
              background:selStudent?.id===s.id?'#00838F':'#F0F6FC',
              color:selStudent?.id===s.id?'#fff':'#00838F',
            }}>
      ...
      {selStudent && (
        <div style={{background:'#E0F7FA',border:'1px solid #00838F22',borderRadius:8,padding:'8px 16px',marginBottom:16,fontSize:12,color:'#00838F',display:'flex',gap:16,flexWrap:'wrap'}}>
      ...
      {loading ? (
        ...
        <>
          {/* Clases de hoy */}
          {clasesHoy.length > 0 && (
            <div style={{background:'#E0F7FA',border:'1px solid #00838F22',borderRadius:12,padding:'14px 18px',marginBottom:20}}>
              <div style={{fontWeight:700,fontSize:13,color:'#00838F',marginBottom:10}}>
                📅 Hoy — {DAYS[todayDay]}
              </div>
          ...
          <div style={{display:'flex',gap:8,marginBottom:16}}>
            {(['grilla','lista'] as const).map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{
                ...
                background:view===v?'#00838F':'#F0F6FC',
                color:view===v?'#fff':'#00838F',
              }}>
                ...
                <thead>
                  <tr>
                    ...
                    {days.map(d=>(
                      <th key={d} style={{
                        ...thStyle,
                        background:d===todayDay?'#00838F':'#F0F6FC',
                    ...
                    return (
                      <tr key={period}>
                        <td style={{...tdStyle,textAlign:'center',background:'#F8FBFF',fontWeight:700,fontSize:11,whiteSpace:'nowrap'}}>
                          <div style={{color:'#00838F'}}>P{period}</div>
                          {firstCell&&<>
                        ...
                        {days.map(day=>{
                          ...
                          return (
                            <td key={day} style={{
                              ...
                              borderColor:day===todayDay?'#00838F33':'#E0EAF5',
                              minWidth:110,
                            }}>
                              {cell?(
                                <div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              {Object.entries(byDay).sort(([a],[b])=>parseInt(a)-parseInt(b)).map(([day,items])=>{
                ...
                return (
                  <div key={day} style={{
                    background:'#fff',
                    border:`1px solid ${isToday?'#00838F':'#CBE0F0'}`,
                    borderRadius:12,overflow:'hidden'
                  }}>
                    <div style={{
                      background:isToday?'#00838F':'#F0F6FC',
                      padding:'10px 16px',fontWeight:700,fontSize:13,
                      color:isToday?'#fff':'#00838F',
                        ...
                        return (
                          <div key={item.id} style={{display:'flex',alignItems:'center',gap:14,padding:'12px 16px',borderTop:'1px solid #F0F6FC'}}>
                            <div style={{textAlign:'center',minWidth:50}}>
                              <div style={{fontSize:11,fontWeight:700,color:'#00838F'}}>P{item.period}</div>
...
const thStyle: React.CSSProperties = {
  ...
  color:'#00838F',textAlign:'center',border:'1px solid #CBE0F0',
}
