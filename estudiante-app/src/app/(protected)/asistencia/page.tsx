...
import { asistenciaApi, AttendanceDay, AttendanceStatus, MyProfile } from '@/modules/asistencia/api'

const GRADES: Record<string, string> = { PRIMERO: '1°', SEGUNDO: '2°', TERCERO: '3°', CUARTO: '4°', QUINTO: '5°', SEXTO: '6°' }
const courseLabel = (c: MyProfile['course']) => (c ? `${GRADES[c.grade] || c.grade} "${c.parallel}"` : '')
...
export default function AsistenciaPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    asistenciaApi.getHistory(profile.id, monthParam)
    ...
    <div className="flex flex-col max-w-lg mx-auto">
      ...
      {profile && (
        <div className="px-4 pt-3">
          <div className="text-[15px] font-semibold text-brand-700">{profile.firstName} {profile.lastName}</div>
          {profile.course && <div className="text-[13px] text-text-secondary">{courseLabel(profile.course)}</div>}
        </div>
      )}
